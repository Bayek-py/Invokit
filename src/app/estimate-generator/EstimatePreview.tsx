      ...
      <div className="p-8 min-h-[700px] flex flex-col">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            {invoice.businessTaxId && <p className="text-sm text-[var(--muted-foreground)] mt-1">ABN/Tax ID: {invoice.businessTaxId}</p>}
          </div>
          <div className="text-right">
            <h2 className="text-3xl font-bold text-amber-600 uppercase tracking-wide">Estimate</h2>
            <p className="text-sm text-[var(--muted-foreground)] mt-2">{invoice.invoiceNumber}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between mb-8">
          <div>
            <h3 className="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wide mb-2">Prepared For</h3>
          ...
          <table className="w-full">
            ...
            <tbody>
              {invoice.items.map((item, idx) => (
                <tr key={item.id} className={idx % 2 === 0 ? "bg-amber-50" : "bg-white"}>
        ...
        <div className="mt-8 flex justify-end">
          <div className="w-72 space-y-2">
            ...
            <div className="flex justify-between text-lg font-bold border-t-2 border-amber-500 pt-2">
              <span>Estimate Total</span>
              <span className="text-amber-600">{formatCurrency(total, curr)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-auto pt-6 text-center">
          <p className="text-xs text-[var(--muted-foreground)]">
            Made with <a href="/" className="text-[var(--brand)] hover:underline font-medium">InvoKit</a> — Free Estimate Generator
          </p>
        </div>
      </div>
    </div>
